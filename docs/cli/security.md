...
Because suppressions can hide standing risk, adding or removing them through agent-run shell commands requires exec approval unless exec is already running with `security="full"` and `ask="off"` for trusted local automation. Recognized read-only file inspections, such as `rg 'security.audit.suppressions' src`, are exempt only when their executable paths satisfy the existing `tools.exec.safeBinTrustedDirs` policy. Unknown file-reader executables stay approval-gated. Existing handling of `openclaw config get/schema/validate` is unchanged. Ordinary exec and node policies still apply; mixed writes, redirections, executable-bearing options, and uncertain analysis keep the extra gate.
