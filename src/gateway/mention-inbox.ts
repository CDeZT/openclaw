...
export function createMentionInbox(params: {
  ...
  function canonicalProfiles(): [string, string][] {
    return [...retainedProfileIds()].map((id) => [id, policy.readProfile(id)?.profileId ?? id]);
  }
  ...
  async function mutate(action: MentionInboxMutation["action"], guard = assertCurrent) {
    ...
    const result = await mutateMentionInbox(
      context,
      { action, now: Date.now(), canonicalProfiles: canonicalProfiles() },
      () => {
        assertCurrent();
  ...
  function retainedPolicyPreparation() {
    ...
    const targets = [...items.values()].map((item) => {
